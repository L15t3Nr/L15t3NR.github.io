...
Olympus can be found [here](/assets/img/Olympus/https://tryhackme.com/room/olympusroom)

![Olympus](/assets/img/Olympus/Olympus.jpeg)
...
![ServerNotFound](/assets/img/Olympus/ServerNotFound.png)

## Hostname
The IP address attempts to resolve to http://olympus.thm. `olympus.thm` will be added to /etc/hosts.


![etcHosts](/assets/img/Olympus/etcHosts.png)

## Manual Inspection
Now the homepage can be seen with a message on screen. 

![HomePage](/assets/img/Olympus/HomePage.png)

![Message](/assets/img/Olympus/Message.png)
...
![Wappalyzer](/assets/img/Olympus/Wappalyzer.png)

Its useful to know that this is running on Ubuntu and the web server is Apache 2.4.41.

![StaticDirectory](/assets/img/Olympus/StaticDirectory.png)

There is a `/static/` directory containing some things. 

![StaticDirectory](/assets/img/Olympus/StaticDirectory2.png)

There is also mention of [Aperi Solve](/assets/img/Olympus/https://www.aperisolve.com/) in the page source. 

![AperiSolve](/assets/img/Olympus/AperiSolve.png)
...
![Victor Alagwu](/assets/img/Olympus/VictorAlagwu.png)
...
![Searchsploit](/assets/img/Olympus/Searchsploit.png)
...
![SQLiError](/assets/img/Olympus/SQLiError.png)
...
Fortunately, there is a repository on [GitHub](/assets/img/Olympus/https://github.com/VictorAlagwu/CMSsite) that contains just that! 

![SearchSource](/assets/img/Olympus/SearchSource.png)
...
Since the injection will take place after a SELECT operator, I will need to use the UNION operator to combine two SELECT statements. Here is a resource for [UNION Operators](/assets/img/Olympus/https://www.techonthenet.com/mysql/union.php) 
...
The extra dash in `-- -` is necessary in MySQL syntax so the "character after the whitespace ... is not trailing and won't be trimmed". Link for more info about that: [MySQL Comment](/assets/img/Olympus/https://blog.raw.pm/en/sql-injection-mysql-comment/)

![UNIONResult](/assets/img/Olympus/UNIONResult.png)
...
![currentdatabase](/assets/img/Olympus/currentdatabase.png)
...
The following should return table information in the expression intended for the title of the post. I used [this](/assets/img/Olympus/https://medium.com/@nyomanpradipta120/sql-injection-union-attack-9c10de1a5635) article for help with the following payloads.

`' union select 1,2,group_concat(table_name),4,5,6,7,8,9,10 from information_schema.tables where table_schema = database() -- -`

![tablesInfo](/assets/img/Olympus/tablesInfo.png)
...
![flagcolumn](/assets/img/Olympus/flagCollumn.png)
...
![UsersTable](/assets/img/Olympus/UsersTable.png)
...
![PrometheusLogin](/assets/img/Olympus/PrometheusLogin.png)

After some exploring I find a place to upload a file on the Profile page, when adding a post, and when adding a user. 

![PrometheusPicUload](/assets/img/Olympus/PromethusPicUpload.png)

![PrometheusAddPost](/assets/img/Olympus/PrometheusAddPost.png)

![AddUserUpload](/assets/img/Olympus/AddUserUpload.png)
...
![FileUploadSearchsploit](/assets/img/Olympus/FileUploadSearchsploit.png)
...
![ForbiddenImg](/assets/img/Olympus/ForbiddenImg.png)
...
![chat](/assets/img/Olympus/chat.png)
...
![chatDatabase](/assets/img/Olympus/chatDatabase.png)
...
![302Found](/assets/img/Olympus/302Found.png)
...
![ChatFuzz](/assets/img/Olympus/ChatFuzz.png)
...
![UploadsDirectory](/assets/img/Olympus/UploadsDirectory.png)
...
![PayloadUpload](/assets/img/Olympus/PayloadUpload.png)

Now to check if its accessible. 

Looks like its not found. 

![FileNotFound](/assets/img/Olympus/FileNotFound.png)
...
![BlingBling](/assets/img/Olympus/BlingBling.png)
...
![SourceFile](/assets/img/Olympus/SourceFile.png)
...
![id_rsa_passphrase](/assets/img/Olympus/id_rsa_passphrase.png)
...
![SSH](/assets/img/Olympus/SSH.png)
...
![cputilsEtcPasswd](/assets/img/Olympus/cputilsEtcPasswd.png)

I should have known this would fail. 

There is a snap directory that is owned by zeus. 

![Snap](/assets/img/Olympus/Snap.png)
...
![ZeusFiles](/assets/img/Olympus/ZeusFiles.png)

None of these writable files have any association with root. 

![ZeusGroup](/assets/img/Olympus/ZeusGroup.png)
...
![OddFile](/assets/img/Olympus/OddFile.png)

## Backdoor
A backdoor?

![Backdoor](/assets/img/Olympus/Backdoor.png)
...
![PasswordAgain](/assets/img/Olympus/PasswordAgain.png)

After submitting, I get a reverse shell as root!!!

![rootShell](/assets/img/Olympus/rootShell.png)

This shell isn't good, so I'll change the root password and login over ssh. 

![ChangePasswdd](/assets/img/Olympus/ChangePasswdd.png)

The root flag is easily found. 

There is a note stating that Prometheus has hidden another flag. So I'll start looking for that. I want to find files with a string matching the "flag{}" format. I took to google to find a command that might do that and came across this [Link](/assets/img/Olympus/https://stackoverflow.com/questions/16956810/how-do-i-find-all-files-containing-specific-text-on-linux)
